...
| `internal/api`                     | 2    | Core business logic (HTTP <-> repository translation) that does not itself touch storage or credentials; reads and writes via the Tier 1 repository interface but the interface's own contract is what's load-bearing, not the handler glue. **Accepted risk (Phase B gate round 5, 2026-07-06 — Codex finding E2):** Tier 2 review depth (overall diff, not line-by-line) still permits a handler-level bug that calls the Tier 1 repository with the wrong ID or command — e.g. a swapped path param mutating or deleting the wrong bookmark — even though the repository interface itself stays correct. Reviewed and accepted as-is for this project's scope (single-user, DELETE-retention methodology build); mitigated procedurally rather than by re-tiering: `internal/api` issues get explicit Pre-Phase F Wire Contract review (see CLAUDE.md's REST Adapter Wire Contract gate) and their own test coverage per issue, which is the intended Tier 2 safety net for handler-glue bugs of exactly this shape. Not escalated to Tier 1 — a full line-by-line review depth is disproportionate to a project of this scope and retention policy. |
