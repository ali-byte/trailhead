...
| `web/` | 2 | none (Go side) — communicates with `internal/api` over REST/JSON only | The user-facing board experience — the brief's "what good looks like" bar (drag-and-drop feel, empty/loading/error states). Toolchain scaffold + locked tests committed at issue #6 Pre-Phase F (2026-08-07): Vite/React/TS/Tailwind, Vitest+RTL+MSW, Playwright, `dnd-kit`. App components (`App.tsx`, `AddBar.tsx`, `Card.tsx`, `dnd/deriveNeighbors.ts`) and the design-token theme are still unbuilt — see docs/issues/06-frontend-board-add-bar.md "UI Contract" — that is Phase F's job. |
